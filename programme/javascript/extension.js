...
function activate(context) {
    ...
    const loadConversationDisposable = vscode.commands.registerCommand('claude-code-chat.loadConversation', (filename) => {
        provider.loadConversation(filename);
    });
    // Register CROD commands
    const showCRODStatsDisposable = vscode.commands.registerCommand('claude-code-chat.showCRODStats', () => {
        provider.showCRODStats();
    });
    const toggleCRODLearningDisposable = vscode.commands.registerCommand('claude-code-chat.toggleCRODLearning', () => {
        provider.toggleCRODLearning();
    });
    ...
    statusBarItem.show();
    context.subscriptions.push(disposable, loadConversationDisposable, showCRODStatsDisposable, toggleCRODLearningDisposable, configChangeDisposable, statusBarItem);
    console.log('Claude Code Chat extension with CROD Neural Network activated successfully!');
}
function deactivate() { }
class ClaudeChatViewProvider {
    extensionUri;
    context;
    ...
    getChildren() {
        ...
        const conversationIndex = this.context.workspaceState.get('claude.conversationIndex', []);
        if (conversationIndex.length > 0) {
            ...
            conversationIndex.slice(0, 20).forEach((conv, index) => {
                const item = new vscode.TreeItem(conv.firstUserMessage.substring(0, 50) + (conv.firstUserMessage.length > 50 ? '...' : ''), vscode.TreeItemCollapsibleState.None);
...
class ClaudeChatProvider {
    ...
    _conversationStartTime;
    _conversationIndex = [];
    _treeProvider;
    _currentClaudeProcess;
    _selectedModel = 'default'; // Default model
    _crodProvider; // CROD Neural Network integration
    _lastUserMessage = '';
    _lastClaudeResponse = '';
    constructor(_extensionUri, _context) {
        ...
        this._crodProvider = new crodChatProvider_1.CRODChatProvider(this._context);
        console.log('CROD Neural Network initialized with Claude Code Chat');
    ...
    show() {
        ...
        this._panel = vscode.window.createWebviewPanel('claudeChat', 'Claude Code Chat', column, {
            enableScripts: true,
        ...
        this._panel.webview.onDidReceiveMessage(message => {
            switch (message.type) {
                ...
                case 'getCRODStats':
                    this._sendCRODStats();
                    return;
                case 'toggleCRODLearning':
                    this._toggleCRODLearning();
                    return;
            }
    ...
    async _sendMessageToClaude(message, planMode, thinkingMode) {
        ...
        // Store user message for CROD learning
        ...
        const nodePath = config.get('wsl.nodePath', '/usr/bin/node');
        ...
        switch (jsonData.type) {
                ...
                if (jsonData.message && jsonData.message.content) {
                    ...
                    for (const content of jsonData.message.content) {
                        if (content.type === 'text' && content.text.trim()) {
                            ...
                            // Store Claude's response for CROD learning
                            this._lastClaudeResponse += content.text.trim() + '\n';
                        }
            ...
            case 'result':
                if (jsonData.subtype === 'success') {
                    ...
                    // CROD learns from the conversation
                    if (this._lastUserMessage && this._lastClaudeResponse) {
                        const context = {
                            sessionId: this._currentSessionId,
                            model: this._selectedModel,
                            tools: jsonData.tools || [],
                        ...
                        this._crodProvider.processConversation(this._lastUserMessage, this._lastClaudeResponse.trim(), context).then(() => {
                            console.log('CROD learned from conversation');
                            const stats = this._crodProvider.getStats();
                            console.log(`CROD Stats: ${stats.parameters} parameters, ${stats.patterns} patterns`);
                        }).catch(err => {
                            console.error('CROD learning error:', err);
                        });
                        // Reset for next conversation
                        this._lastClaudeResponse = '';
                    }
                }
                break;
        }
    ...
    newSessionOnConfigChange() {
        // Start a new session due to configuration change
        this._newSession();
        // Show notification to user
        vscode.window.showInformationMessage('WSL configuration changed. Started a new Claude session.', 'OK');
    ...
    _handleLoginRequired() {
        ...
        terminal.show();
        // Show info message
        vscode.window.showInformationMessage('Please login to Claude in the terminal, then come back to this chat to continue.', 'OK');
    ...
    async _createBackupCommit(userMessage) {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    ...
    async _restoreToCommit(commitSha) {
        try {
    ...
    showCRODStats() {
        // Show the webview if not already open
        this.show();
        // Send CROD stats
        setTimeout(() => {
            this._sendCRODStats();
    ...
    toggleCRODLearning() {
        // Show the webview if not already open
        this.show();
        // Toggle CROD learning
        setTimeout(() => {
            this._toggleCRODLearning();
        }, 100);
    }
    _sendConversationList() {
        this._panel?.webview.postMessage({
            type: 'conversationList',
            data: this._conversationIndex
        });
    }
    async _sendWorkspaceFiles(searchTerm) {
        try {
            // Always get all files and filter on the backend for better search results
            const files = await vscode.workspace.findFiles('**/*', '{**/node_modules/**,**/.git/**,**/dist/**,**/build/**,**/.next/**,**/.nuxt/**,**/target/**,**/bin/**,**/obj/**}', 500 // Reasonable limit for filtering
            );
    ...
    _updateConversationIndex(filename, conversationData) {
        // Extract first and last user messages
        const userMessages = conversationData.messages.filter((m) => m.messageType === 'userInput');
    ...
    _sendCurrentSettings() {
        ...
        const settings = {
            ...
            'wsl.nodePath': config.get('wsl.nodePath', '/usr/bin/node'),
    ...
    _openModelTerminal() {
        ...
        terminal.show();
        // Show info message
        vscode.window.showInformationMessage('Check the terminal to update your default model configuration. Come back to this chat here after making changes.', 'OK');
        // Send message to UI about terminal
        this._panel?.webview.postMessage({
            type: 'terminalOpened',
    ...
    _executeSlashCommand(command) {
        ...
        terminal.show();
        // Show info message
        vscode.window.showInformationMessage(`Executing /${command} command in terminal. Check the terminal output and return when ready.`, 'OK');
        // Send message to UI about terminal
        this._panel?.webview.postMessage({
            type: 'terminalOpened',
    ...
    _sendCRODStats() {
        const stats = this._crodProvider.getStats();
        const visualization = this._crodProvider.getNetworkVisualization();
        this._panel?.webview.postMessage({
            type: 'crodStats',
            data: {
                stats: stats,
        ...
        this._sendAndSaveMessage({
            type: 'output',
            data: `🧠 CROD Neural Network Status:\n\n${visualization}\n\nPatterns learned: ${stats.patterns}`
        });
    }
    _toggleCRODLearning() {
        const isLearning = this._crodProvider.toggleLearning();
        const status = isLearning ? 'enabled' : 'disabled';
        vscode.window.showInformationMessage(`CROD learning ${status}`);
        this._sendAndSaveMessage({
            type: 'output',
            data: `🧠 CROD learning has been ${status}`
        });
    }
    dispose() {
        if (this._panel) {
