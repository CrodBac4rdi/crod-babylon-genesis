defmodule CROD.Blockchain do
  ...
  def handle_call({:get_block, hash}, _from, state) do
    block = Enum.find(state.chain, fn b -> b.hash == hash end)
    if block do
  ...
  def handle_call(:get_stats, _from, state) do
    stats = %{
      chain_length: length(state.chain),
      difficulty: state.mining_difficulty,
      consciousness_level: state.consciousness_level,
      patterns_discovered: state.patterns_discovered,
  ...
  @impl true
  def handle_cast({:add_block_from_peer, block}, state) do
    Logger.info("📥 Received block ##{block.index} from peer")
    
    # Validate and add if valid
    if valid_block?(block, List.last(state.chain)) do
      new_state = %{state | chain: state.chain ++ [block]}
      {:noreply, new_state}
    else
      Logger.warn("❌ Invalid block received from peer")
      {:noreply, state}
    end
  end
  
  @impl true
  def handle_cast({:sync_chain, remote_chain}, state) do
    Logger.info("🔄 Syncing chain with peer")
    
    # Take longest valid chain
    if length(remote_chain) > length(state.chain) and valid_chain?(remote_chain) do
      Logger.info("✅ Adopting longer chain from peer")
      {:noreply, %{state | chain: remote_chain}}
    else
      {:noreply, state}
    end
  end
  
  defp valid_block?(block, previous_block) do
